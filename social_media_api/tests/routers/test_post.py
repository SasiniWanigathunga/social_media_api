...
async def create_post(content: str, async_client: AsyncClient):
    response = await async_client.post("/posts", json={"content": content})
    return response.json()

async def create_comment(comments: str, post_id: int, async_client: AsyncClient):
    response = await async_client.post("/comments", json={"comments": comments, "post_id": post_id})
    return response.json()

@pytest.fixture()
async def created_post(async_client: AsyncClient):
    return await create_post("Test post content", async_client)

@pytest.fixture()
async def created_comment(async_client: AsyncClient, created_post: dict):
    return await create_comment("Test comment content", created_post["id"], async_client)
...
async def test_create_post(async_client: AsyncClient):
    ...
    assert {"id": 1, "content": content}.items() <= response.json().items()
...
async def test_create_comment(async_client: AsyncClient, created_post: dict):
    ...
    response = await async_client.post("/comments", json={"comments": comments, "post_id": created_post["id"]})
    
    assert response.status_code == 201
    assert {"id": 1, "comments": comments, "post_id": created_post["id"]}.items() <= response.json().items()
