...
from social_media_api.database import database, post_table
from social_media_api.tasks import send_simple_email, APIResponseError, _generate_cute_creature_api, generate_and_add_to_post


@pytest.mark.anyio
async def test_send_simple_email_success(mock_httpx_client):
...
async def test_generate_cute_creature_api_success(mock_httpx_client):
    json_data = {"output_url": "https://example.com/cute_creature.jpg"}
    ...
    result = await _generate_cute_creature_api("A cute creature")
...
async def test_generate_cute_creature_api_server_error(mock_httpx_client):
    ...
    with pytest.raises(APIResponseError, match="API request failed with status code 500"):
        await _generate_cute_creature_api("A cute creature")
...
async def test_generate_and_add_to_post_success(
    ...
    await generate_and_add_to_post(
        ...
        post_url=f"posts/{created_post['id']}",
        database=db,
        prompt="A cute creature",
    )
